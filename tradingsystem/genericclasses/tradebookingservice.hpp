...
public:
  ...
  // ctir fom execution trade
  Trade(const ExecutionOrder<T>& _exec_order, const string& _book);
 ...
 */
template<typename T>
class TradeBookingService : public ListenedService<string,Trade <T> >
{
...
template<typename T>
Trade<T>::Trade(const ExecutionOrder<T>& _exec_order, const string& _book) :
  product(_exec_order.GetProduct())
{
  tradeId = _exec_order.GetOrderId();
  price = _exec_order.GetPrice();
  book = _book;
  quantity = _exec_order.GetVisibleQuantity();
  side = (_exec_order.GetSide() == PricingSide::BID) ? Side::BUY : Side::SELL;
}
