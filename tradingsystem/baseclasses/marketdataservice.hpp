...
public:
  ...
  double GetTopAgainstPrice(PricingSide side);
    
  double GetTopSpread();
    
  long GetTopAgainstQuantity(PricingSide side);
 ...
 */
template<typename T>
class MarketDataService : public ListenedService<string,OrderBook <T> >
{
...
template<typename T>
double OrderBook<T>:: GetTopAgainstPrice(PricingSide side){
    if (side == PricingSide::OFFER) return bidStack[0].GetPrice();
    else return offerStack[0].GetPrice();
}

template<typename T>
double OrderBook<T>::GetTopSpread(){
    return offerStack[0].GetPrice() -  bidStack[0].GetPrice();
}

template<typename T>
long OrderBook<T>:: GetTopAgainstQuantity(PricingSide side){
    if (side == PricingSide::OFFER) return bidStack[0].GetQuantity();
    else return offerStack[0].GetQuantity();
}
