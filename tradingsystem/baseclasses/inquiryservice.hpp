...
public:
  ...
  // Update the state
  void UpdateState(InquiryState _state);
 ...
 */
template<typename T>
class InquiryService : public ListenedService<string,Inquiry <T> >
{
...
template<typename T>
void Inquiry<T>::UpdateState(InquiryState _state)
{
  state = _state;
}
